function arrowHandle = arrow3D(pos, deltaValues, colorCode, stemRatio, LineWidth, ArrowHeadWidth)
% arrowHandle = arrow3D(pos, deltaValues, colorCode, stemRatio, LineWidth, ArrowHeadWidth) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
...
%    Original downloaded from: https://www.mathworks.com/matlabcentral/fileexchange/12285-3d-quiver-with-volumized-arrows
%    Modified: June 2022 by Tom Okell to include LineWidth and ArrowWidth options    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if nargin<2 || nargin>6 % T.O.     
    ...
    %******************************************* CYLINDER == STEM *********************************************
    % T.O. Set linewidth here
    if nargin < 5; % If not specified, use default as before
    ...
    % T.O. Set the arrow head width
    if nargin < 6; % Use default
      coneRadius = cylinderRadius*1.5;
    else
      coneRadius = ArrowHeadWidth/2;
    end
    incr = 4;  %---- Steps of cone increments
    coneincr = coneRadius/incr;    
    [coneX, coneY, coneZ] = cylinder(coneRadius*2:-coneincr:0);  %---------- CONE %T.O.
    ...
    hold on;  
    % T.O. Make handle invisible to prevent two entries in the legend
    hHead = surf(headX, headY, headZ, 'FaceColor', colorCode, 'EdgeColor', 'none','HandleVisibility','off'); 
